...
public class NativeEngineKnnVectorQuery extends Query {
    ...
    private final boolean isInnerHitQuery;
    ...
    private List<Map<Integer, Float>> retrieveAll(
        final IndexSearcher indexSearcher,
        List<LeafReaderContext> leafReaderContexts,
        KNNWeight knnWeight,
        List<Map<Integer, Float>> perLeafResults
    ) throws IOException {
        List<Callable<Map<Integer, Float>>> nestedQueryTasks = new ArrayList<>(leafReaderContexts.size());
        for (int i = 0; i < perLeafResults.size(); i++) {
            LeafReaderContext leafReaderContext = leafReaderContexts.get(i);
            int finalI = i;
            nestedQueryTasks.add(() -> {
                Bits queryFilter = queryUtils.createBits(leafReaderContext, knnWeight.getFilterWeight());
    ...
    private List<Map<Integer, Float>> doRescore(
        ...
        for (int i = 0; i < perLeafResults.size(); i++) {
            ...
            int finalI = i;
            rescoreTasks.add(() -> {
                DocIdSetIterator matchedDocs = ResultUtil.resultMapToDocIds(perLeafResults.get(finalI));
                final ExactSearcher.ExactSearcherContext exactSearcherContext = ExactSearcher.ExactSearcherContext.builder()
