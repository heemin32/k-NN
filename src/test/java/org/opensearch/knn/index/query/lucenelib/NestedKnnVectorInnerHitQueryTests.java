...
public class NestedKnnVectorInnerHitQueryTests extends TestCase {
    ...
    public void testCreateWeight_whenCalled_thenSucceed() {
        ...
        float boost = 1.f;
        DocAndScoreQuery docAndScoreQuery = mock(DocAndScoreQuery.class);
        ...
        when(queryFilterBits.get(1)).thenReturn(false);
        when(queryFilterBits.get(2)).thenReturn(true);
        ...
        when(queryUtils.getAllSiblings(any(), any(), any())).thenReturn(allSiblings);
        when(queryUtils.createDocAndScoreQuery(eq(indexReader), any())).thenReturn(finalQuery);

        // Run
        NestedKnnVectorInnerHitQuery query = new NestedKnnVectorInnerHitQuery(internalQuery, queryUtils);
        ...
        verify(queryUtils).getAllSiblings(leafReaderContext1, perLeafResults.get(0).keySet(), parentFilter);
        verify(queryUtils).getAllSiblings(leafReaderContext2, perLeafResults.get(1).keySet(), parentFilter);
